...
function flattenRoutes(definitions, base, layoutChain = []) {
  const resolved = [];
  for (const def of definitions) {
    if (def._type === 'layout') {
      const nextChain = def.moduleFactory ? [...layoutChain, def.moduleFactory] : layoutChain;
...
async function buildLevel(mod, params, queryParams, request) {
  let loaderData;
  if (mod.loader) {
...
class Router {
  ...
  #onMatch;
  #renderToken = 0;
  constructor(config, base = '/', onMatch) {
  ...
  async #loadModules(leafMatch, queryParams, token, signal) {
    const {
      data: route
    } = leafMatch;
  ...
  async #buildAndEmit(url, mods, params, token, signal) {
  ...
  async #handleAction(url, matches, token, formData, signal) {
  ...
  async #renderForUrl(url, matches, token, signal) {
    const result = await this.#loadModules(matches[0], url.searchParams, token, signal);
    if (!result) return;
